...
inline void accumulatorApply(const Accumulator& acc, Target& target, Key&& key, long value, Int<1>) {
    for (auto &it : std::get<0>(acc).store) {
        target[std::tuple_cat(std::get<0>(it), key)] += std::get<1>(it) * value;
    }
}
...
struct RelationMap : public Relation<Id, Map<Keys...>> {
    ...
    RelationMap& operator+=(const Accumulator<Args...>& acc) {
        if (!accumulatorIsZero(acc.values, Int<sizeof...(Args)>()))
            accumulatorApply(acc.values, this->store, std::tuple<>(), 1L, Int<sizeof...(Args)>());
        return *this;
    }
...
template <size_t Id, typename... Keys>
using RingRelation = RelationMap<Id, Keys...>;
