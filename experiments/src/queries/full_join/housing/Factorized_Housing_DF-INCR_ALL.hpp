...
namespace dbtoaster {
  ...
  struct data_t : tlq_t{
    ...
    void on_batch_update_HOUSE(const std::vector<HOUSE_entry>::iterator &begin, const std::vector<HOUSE_entry>::iterator &end) {
      ...
      { //foreach
        for (std::vector<HOUSE_entry>::iterator e1 = begin; e1 != end; e1++) {
          ...
          DELTA_TMP1.addOrDelOnZero(key_h2_house, payload_h2_house_map);
        }
      }

      { //foreach
        KEY_H2_HOUSE* e7 = DELTA_TMP1.head;
        while (e7) {
          ...
          TMP1.addOrDelOnZero(*e7, e7->__av);
          e7 = e7->nxt;
        }
      }
    }
    ...
    void on_batch_update_SHOP(const std::vector<SHOP_entry>::iterator &begin, const std::vector<SHOP_entry>::iterator &end) {
      ...
      DELTA_TMP2.clear();
      { //foreach
        for (std::vector<SHOP_entry>::iterator e2 = begin; e2 != end; e2++) {
          ...
          DELTA_TMP2.addOrDelOnZero(key_s2_shop, payload_s2_shop_map);
        }
      }

      { //foreach
        KEY_S2_SHOP* e7 = DELTA_TMP2.head;
        while (e7) {
          DOUBLE_TYPE postcode = e7->postcode;
          const PAYLOAD_S2_SHOP_map& payload_s2_shop_map = e7->__av;
          const PAYLOAD_H2_HOUSE_map& payload_h2_house_map = TMP1.getValueOrDefault(key_h2_house.modify(postcode));
          if (payload_h2_house_map.count() > 0) {
            const PAYLOAD_I2_INSTITUTION_map& payload_i2_institution_map = TMP3.getValueOrDefault(key_i2_instution.modify(postcode));
            if (payload_i2_institution_map.count() > 0) {
              const PAYLOAD_R2_RESTAURANT_map& payload_r2_restaurant_map = TMP4.getValueOrDefault(key_r2_restaurant.modify(postcode));
              if (payload_r2_restaurant_map.count() > 0) {
          ...
          TMP2.addOrDelOnZero(*e7, e7->__av);
          e7 = e7->nxt;
        }
      }      
    }
    ...
    void on_batch_update_INSTITUTION(const std::vector<INSTITUTION_entry>::iterator &begin, const std::vector<INSTITUTION_entry>::iterator &end) {
      ...
      DELTA_TMP3.clear();
      { //foreach
        for (std::vector<INSTITUTION_entry>::iterator e3 = begin; e3 != end; e3++) {
          ...
          DELTA_TMP3.addOrDelOnZero(key_i2_instution, payload_i2_institution_map);
        }
      }


      { //foreach
        KEY_I2_INSTITUTION* e7 = DELTA_TMP3.head;
        while (e7) {
          DOUBLE_TYPE postcode = e7->postcode;
          const PAYLOAD_I2_INSTITUTION_map& payload_i2_institution_map = e7->__av;
          const PAYLOAD_H2_HOUSE_map& payload_h2_house_map = TMP1.getValueOrDefault(key_h2_house.modify(postcode));
          if (payload_h2_house_map.count() > 0) {
            const PAYLOAD_S2_SHOP_map& payload_s2_shop_map = TMP2.getValueOrDefault(key_s2_shop.modify(postcode));
            if (payload_s2_shop_map.count() > 0) {
          ...
          TMP3.addOrDelOnZero(*e7, e7->__av);
          e7 = e7->nxt;
        }
      }
    }
    ...
    void on_batch_update_RESTAURANT(const std::vector<RESTAURANT_entry>::iterator &begin, const std::vector<RESTAURANT_entry>::iterator &end) {
      ...
      DELTA_TMP4.clear();
      { //foreach
        for (std::vector<RESTAURANT_entry>::iterator e4 = begin; e4 != end; e4++) {
          ...
          DELTA_TMP4.addOrDelOnZero(key_r2_restaurant, payload_r2_restaurant_map);
        }
      }

      { //foreach
        KEY_R2_RESTAURANT* e7 = DELTA_TMP4.head;
        while (e7) {
          DOUBLE_TYPE postcode = e7->postcode;
          const PAYLOAD_R2_RESTAURANT_map& payload_r2_restaurant_map = e7->__av;
          const PAYLOAD_H2_HOUSE_map& payload_h2_house_map = TMP1.getValueOrDefault(key_h2_house.modify(postcode));
          if (payload_h2_house_map.count() > 0) {
            const PAYLOAD_S2_SHOP_map& payload_s2_shop_map = TMP2.getValueOrDefault(key_s2_shop.modify(postcode));
            if (payload_s2_shop_map.count() > 0) {
          ...
          TMP4.addOrDelOnZero(*e7, e7->__av);
          e7 = e7->nxt;
        }
      }
    }
    ...
    void on_batch_update_DEMOGRAPHICS(const std::vector<DEMOGRAPHICS_entry>::iterator &begin, const std::vector<DEMOGRAPHICS_entry>::iterator &end) {
      ...
      DELTA_TMP5.clear();
      { //foreach
        for (std::vector<DEMOGRAPHICS_entry>::iterator e5 = begin; e5 != end; e5++) {
          ...
          DELTA_TMP5.addOrDelOnZero(key_d2_demographics, payload_d2_demographics_map);
        }
      }

      { //foreach
        KEY_D2_DEMOGRAPHICS* e7 = DELTA_TMP5.head;
        while (e7) {
          DOUBLE_TYPE postcode = e7->postcode;
          const PAYLOAD_D2_DEMOGRAPHICS_map& payload_d2_demographics_map = e7->__av;
          const PAYLOAD_H2_HOUSE_map& payload_h2_house_map = TMP1.getValueOrDefault(key_h2_house.modify(postcode));
          if (payload_h2_house_map.count() > 0) {
            const PAYLOAD_S2_SHOP_map& payload_s2_shop_map = TMP2.getValueOrDefault(key_s2_shop.modify(postcode));
            if (payload_s2_shop_map.count() > 0) {
          ...
          TMP5.addOrDelOnZero(*e7, e7->__av);
          e7 = e7->nxt;
        }
      }
    }    
    ...
    void on_batch_update_TRANSPORT(const std::vector<TRANSPORT_entry>::iterator &begin, const std::vector<TRANSPORT_entry>::iterator &end) {
      ...
      DELTA_TMP6.clear();
      { //foreach
        for (std::vector<TRANSPORT_entry>::iterator e6 = begin; e6 != end; e6++) {
          ...
          DELTA_TMP6.addOrDelOnZero(key_t2_transport, payload_t2_transport_map);
        }
      }

      { //foreach
        KEY_T2_TRANSPORT* e7 = DELTA_TMP6.head;
        while (e7) {
          DOUBLE_TYPE postcode = e7->postcode;
          const PAYLOAD_T2_TRANSPORT_map& payload_t2_transport_map = e7->__av;
          const PAYLOAD_H2_HOUSE_map& payload_h2_house_map = TMP1.getValueOrDefault(key_h2_house.modify(postcode));
          if (payload_h2_house_map.count() > 0) {
            const PAYLOAD_S2_SHOP_map& payload_s2_shop_map = TMP2.getValueOrDefault(key_s2_shop.modify(postcode));
            if (payload_s2_shop_map.count() > 0) {
          ...
          TMP6.addOrDelOnZero(*e7, e7->__av);
          e7 = e7->nxt;
        }
      }
    }
