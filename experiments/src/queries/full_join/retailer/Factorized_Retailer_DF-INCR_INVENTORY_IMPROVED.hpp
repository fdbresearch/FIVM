...
namespace dbtoaster {
  ...
  struct PAYLOAD_KSN_INVENTORY_ITEM {
    DOUBLE_TYPE ksn; DOUBLE_TYPE inventoryunits;
  ...
  struct PAYLOAD_KSN_INVENTORY_ITEM_mapkey0123_idxfn {
    ...
    FORCE_INLINE static bool equals(const PAYLOAD_KSN_INVENTORY_ITEM& x, const PAYLOAD_KSN_INVENTORY_ITEM& y) {
      return x.ksn == y.ksn && x.inventoryunits == y.inventoryunits;
    }
  };
  ...
  struct PAYLOAD_DATEID_INVENTORY_ITEM_WEATHER {
    long dateid; long __av; PAYLOAD_DATEID_INVENTORY_ITEM_WEATHER* nxt; PAYLOAD_DATEID_INVENTORY_ITEM_WEATHER* prv;
  ...
  struct PAYLOAD_ZIP_LOCATION_CENSUS {
    long zip; long __av; PAYLOAD_ZIP_LOCATION_CENSUS* nxt; PAYLOAD_ZIP_LOCATION_CENSUS* prv;
  ...
  struct PAYLOAD_LOCN {
    long locn; long __av; PAYLOAD_LOCN* nxt; PAYLOAD_LOCN* prv;
  ...
  struct KEY_RGNCD_LOCATION {
    ...
    FORCE_INLINE KEY_RGNCD_LOCATION& e(const long c1) { zip = c1;  return *this; }
    template<class Archive>
    void serialize(Archive& ar, const unsigned int version) const {
      ar << ELEM_SEPARATOR;
      DBT_SERIALIZATION_NVP(ar, locn);
    ...
    void on_batch_update_INVENTORY(const std::vector<INVENTORY_entry>::iterator &begin, const std::vector<INVENTORY_entry>::iterator &end) {
      ...
      { //foreach
        for (std::vector<DELTA_INVENTORY_entry>::iterator e1 = begin; e1 != end; e1++) {
          ...
          if (pItem.count() > 0) {
            ...
            payload_ksn_inventory_item.inventoryunits = inventoryunits;
            payload_ksn_inventory_item.__av = v1 * pItem.count();
      ...
      { //foreach
        ...
        while (e3) {
          ...
          AGG_InventoryItem.addOrDelOnZero(*e3, e3->__av);          
          e3 = e3->nxt;
        }
      }
      ...
      {//foreach
        ...
        while (e4) {
          ...
          AGG_InventoryItemWeather.addOrDelOnZero(*e4, e4->__av);
          e4 = e4->nxt;
        }        
      }
    }   
