...
------------------- TRIGGERS --------------------
ON + LINEITEM (orderkey, partkey, suppkey, linenumber, quantity, extendedprice, discount, tax, returnflag, linestatus, shipdate, commitdate, receiptdate, shipinstruct, shipmode, comment) {
  V_returnflag_L1(TPCH1Payload)[][returnflag, linestatus]<Local>  +=  ((1 * {[date: date]('1997-09-01') >= shipdate}) * [lift: TPCH1Payload](quantity, extendedprice, (extendedprice * (1 + (-1 * discount))), ((extendedprice * (1 + (-1 * discount))) * (1 + tax)), discount));
}
