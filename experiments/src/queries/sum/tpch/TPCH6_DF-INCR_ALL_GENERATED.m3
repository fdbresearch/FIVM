...
ON BATCH UPDATE OF LINEITEM {
  V_l_orderkey_L1(double)[][]<Local>  +=  AggSum([],
    ((((((((DELTA LINEITEM)(l_orderkey, l_partkey, l_suppkey, l_linenumber, l_quantity, l_extendedprice, l_discount, l_tax, l_returnflag, l_linestatus, l_shipdate, l_commitdate, l_receiptdate, l_shipinstruct, l_shipmode, l_comment) * {l_shipdate >= [date: date]('1994-01-01')}) * {[date: date]('1995-01-01') > l_shipdate}) * {l_discount >= 0.05}) * {0.07 >= l_discount}) * {24 > l_quantity}) * l_discount) * l_extendedprice)
  );
}
