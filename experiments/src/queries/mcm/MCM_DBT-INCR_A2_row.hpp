...
namespace dbtoaster {
  ...
  struct DELTA_BASE_A2_entry {
    long i; long j; DOUBLE_TYPE __av; DELTA_BASE_A2_entry* nxt; DELTA_BASE_A2_entry* prv;
    explicit DELTA_BASE_A2_entry() : nxt(nullptr), prv(nullptr) { }
    explicit DELTA_BASE_A2_entry(const long c0, const long c1, const DOUBLE_TYPE c2) { i = c0; j = c1; __av = c2;  }
    DELTA_BASE_A2_entry(const DELTA_BASE_A2_entry& other) : i(other.i), j(other.j), __av(other.__av), nxt(nullptr), prv(nullptr) { }
    DELTA_BASE_A2_entry(const std::vector<std::string>& f, const DOUBLE_TYPE v) { /* if (f.size() < 2) return; */ i = std::stol(f[0]); j = std::stol(f[1]); __av = v; nxt = nullptr; prv = nullptr; }
    FORCE_INLINE DELTA_BASE_A2_entry& modify(const long c0, const long c1) { i = c0; j = c1;  return *this; }
    FORCE_INLINE DELTA_BASE_A2_entry& modify0(const long c0) { i = c0;  return *this; }
    template<class Archive>
    void serialize(Archive& ar, const unsigned int version) const {
      ar << ELEM_SEPARATOR;
      DBT_SERIALIZATION_NVP(ar, i);
  ...
  struct DELTA_BASE_A2_mapkey01_idxfn {
    FORCE_INLINE static size_t hash(const DELTA_BASE_A2_entry& e) {
      size_t h = 0;
      hash_combine(h, e.i);
      hash_combine(h, e.j);
      return h;
    }
    FORCE_INLINE static bool equals(const DELTA_BASE_A2_entry& x, const DELTA_BASE_A2_entry& y) {
      return x.i == y.i && x.j == y.j;
    }
  };
  
  struct DELTA_BASE_A2_mapkey0_idxfn {
    FORCE_INLINE static size_t hash(const DELTA_BASE_A2_entry& e) {
      size_t h = 0;
      hash_combine(h, e.i);
      return h;
  ...
  typedef MultiHashMap<DELTA_BASE_A2_entry, DOUBLE_TYPE, 
    PrimaryHashIndex<DELTA_BASE_A2_entry, DELTA_BASE_A2_mapkey01_idxfn>, 
    SecondaryHashIndex<DELTA_BASE_A2_entry, DELTA_BASE_A2_mapkey0_idxfn>
  > DELTA_BASE_A2_map;
  ...
  struct data_t : tlq_t{
    ...
    void on_batch_update_A2(const std::vector<A2_entry>::iterator &begin, const std::vector<A2_entry>::iterator &end) {
      ...
      for (int j = 0; j < matrixSize; j++) {
        ...
        DELTA_A2.addOrDelOnZero(de2, de2.__av);
      }     
      tN += DELTA_A2.count();
      ...
      { //foreach
        ...
        while (e2) {
          ...
          { //slice
            const SecondaryIdxNode<DELTA_BASE_A2_entry>* n3_head = static_cast<const SecondaryIdxNode<DELTA_BASE_A2_entry>*>(DELTA_A2.slice(de2.modify0(k), 0));
            const SecondaryIdxNode<DELTA_BASE_A2_entry>* n3 = n3_head;
            DELTA_BASE_A2_entry* e3;
            while (n3) {
              e3 = n3->obj;
              long l = e3->j;
      ...
      { //temp foreach
        DELTA_BASE_A2_entry* e2 = DELTA_A2.head;
        while(e2) {
          long k = e2->i;
          long l = e2->j;
