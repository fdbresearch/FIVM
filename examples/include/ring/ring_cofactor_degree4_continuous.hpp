#ifndef RING_COFACTOR_DEGREE4_CONTINUOUS_HPP
#define RING_COFACTOR_DEGREE4_CONTINUOUS_HPP
...
struct RingCofactor {
    ...
    explicit RingCofactor(Args&&... args) : count(1), arr { args... } {
        ...
        T *out5 = out4 + array_size_per_degree(SZ, 4);
        T *out6 = out5 + array_size_per_degree(SZ, 5);
        T *out7 = out6 + array_size_per_degree(SZ, 6);
        T *out8 = out7 + array_size_per_degree(SZ, 7);
                    ...
                    *out3++ = arr[i] * arr[j] * arr[k];
                    for (size_t l = k; l < SZ; l++) {
                        *out4++ = arr[i] * arr[j] * arr[k] * arr[l];
                        for (size_t m = l; m < SZ; m++) {
                            *out5++ = arr[i] * arr[j] * arr[k] * arr[l] * arr[m];
                            for (size_t n = m; n < SZ; n++) {
                                *out6++ = arr[i] * arr[j] * arr[k] * arr[l] * arr[m] * arr[n];
                                for (size_t o = n; o < SZ; o++) {
                                    *out7++ = arr[i] * arr[j] * arr[k] * arr[l] * arr[m] * arr[n] * arr[o];
                                    for (size_t p = o; p < SZ; p++) {
                                        *out8++ = arr[i] * arr[j] * arr[k] * arr[l] * arr[m] * arr[n] * arr[o] * arr[p];
                                    }
                                }
                            }
                        }
...
#endif /* RING_COFACTOR_DEGREE4_CONTINUOUS_HPP */