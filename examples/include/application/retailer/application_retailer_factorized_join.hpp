...
class RetailerFactorizedJoinApplication : public Application {
 public:
  ...
  void onEndProcessing(const dbtoaster::data_t& data,
    ...
    // cout << "  sizeof(V_locn_IIWLC1) = " <<
    // data.get_V_locn_IIWLC1().store.size() << endl; cout << "
    // sizeof(V_dateid_IIW1) = " << data.get_V_dateid_IIW1().count() << endl;
    // cout << "  sizeof(V_ksn_II1) = " << data.get_V_ksn_II1().count() << endl;
    // cout << "  sizeof(V_subcategory_I1) = " <<
    // data.get_V_subcategory_I1().count() << endl; cout << "  sizeof(V_rain_W1)
    // = " << data.get_V_rain_W1().count() << endl; cout << "  sizeof(V_zip_LC1)
    // = " << data.get_V_zip_LC1().count() << endl; cout << "
    // sizeof(V_rgn_cd_L1) = " << data.get_V_rgn_cd_L1().count() << endl; cout
    // << "  sizeof(V_population_C1) = " << data.get_V_population_C1().count()
    // << endl;
    ...
    for (auto& t1 : viewIIWLC.store) {
      ...
      {  // For each V_dateid_IIW1(locn) -> dateid
        ...
        const auto& rel2 =
            data.get_V_dateid_IIW1().getValueOrDefault(e2.modify(locn));
        for (auto& t2 : rel2.store) {
          ...
          V_ksn_II1_entry e3;
          const auto& rel3 =
              data.get_V_ksn_II1().getValueOrDefault(e3.modify(locn, dateid));
          for (auto& t3 : rel3.store) {
            auto& key3 = t3.first;
            long value3 = t3.second;
            auto ksn = std::get<0>(key3);

            // For each V_subcategory_I1(ksn) -> (subcategory, category,
            // categoryCluster, prize)
            V_subcategory_I1_entry e4;
            const auto& rel4 =
                data.get_V_subcategory_I1().getValueOrDefault(e4.modify(ksn));
            for (auto& t4 : rel4.store) {
              auto& key4 = t4.first;
              long value4 = t4.second;
              long value34 = value3 * value4;

              // For each V_rain_W1(locn, dateid) -> (rain, snow, maxtemp,
              // mintemp, meanwind, thunder)
              V_rain_W1_entry e5;
              const auto& rel5 = data.get_V_rain_W1().getValueOrDefault(
                  e5.modify(locn, dateid));
              for (auto& t5 : rel5.store) {
                ...
                V_zip_LC1_entry e6;
                const auto& rel6 =
                    data.get_V_zip_LC1().getValueOrDefault(e6.modify(locn));
                for (auto& t6 : rel6.store) {
                  auto& key6 = t6.first;
                  auto zip = std::get<0>(key6);

                  // For each V_rgn_cd_L1(locn, zip) -> (rgn_cd, clim_zn_nbr,
                  ...
                  const auto& rel7 = data.get_V_rgn_cd_L1().getValueOrDefault(
                      e7.modify(locn, zip));
                  for (auto& t7 : rel7.store) {
                    ...
                    // For each V_population_C1(zip) -> (population, white,
                    // asian, pacific, blackafrican, medianage,
                    ...
                    const auto& rel8 =
                        data.get_V_population_C1().getValueOrDefault(
                            e8.modify(zip));
...
std::unique_ptr<Application> createApplication() {
  return std::make_unique<RetailerFactorizedJoinApplication>();
}
