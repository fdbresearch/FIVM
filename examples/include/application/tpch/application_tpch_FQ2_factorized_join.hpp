...
class TpchFQ2FactorizedJoinApplication : public Application {
 public:
  ...
  void onEndProcessing(const dbtoaster::data_t& data,
    ...
    // cout << "  sizeof(V_orderkey_LPCNO1) = " <<
    // data.get_V_orderkey_LPCNO1().store.size() << endl; cout << "
    // sizeof(V_partkey_LP1) = " << data.get_V_partkey_LP1().count() << endl;
    // cout << "  sizeof(V_l_suppkey_L1) = " <<
    // data.get_V_l_suppkey_L1().count() << endl; cout << "  sizeof(V_p_name_P1)
    // = " << data.get_V_p_name_P1().count() << endl; cout << "
    // sizeof(V_custkey_CNO1) = " << data.get_V_custkey_CNO1().count() << endl;
    // cout << "  sizeof(V_nationkey_CN1) = " <<
    // data.get_V_nationkey_CN1().count() << endl; cout << " sizeof(V_c_name_C1)
    // = " << data.get_V_c_name_C1().count() << endl; cout << "
    // sizeof(V_n_name_N1) = " << data.get_V_n_name_N1().count() << endl; cout
    // << "  sizeof(V_o_orderstatus_O1) = " <<
    // data.get_V_o_orderstatus_O1().count() << endl;
        ...
        for (auto& t2 : rel2.store) {
          ...
          for (auto& t3 : rel3.store) {
            ...
            for (auto& t4 : rel4.store) {
              ...
              const auto& rel5 = data.get_V_custkey_CNO1().getValueOrDefault(
                  e5.modify(orderkey));
              for (auto& t5 : rel5.store) {
                ...
                const auto& rel6 = data.get_V_nationkey_CN1().getValueOrDefault(
                    e6.modify(custkey));
                for (auto& t6 : rel6.store) {
                  auto& key6 = t6.first;
                  auto nationkey = std::get<0>(key6);

                  // For each V_c_name_C1(custkey, nationkey) -> (c_name,
                  // c_address, c_phone, c_acctbal, c_mktsegment, c_comment)
                  V_c_name_C1_entry e7;
                  const auto& rel7 = data.get_V_c_name_C1().getValueOrDefault(
                      e7.modify(custkey, nationkey));
                  for (auto& t7 : rel7.store) {
                    auto& key7 = t7.first;
                    long value7 = t7.second;
                    long value347 = value34 * value7;

                    // For each V_n_name_N1(nationkey) -> (n_name, n_regionkey,
                    // n_comment)
                    V_n_name_N1_entry e8;
                    const auto& rel8 = data.get_V_n_name_N1().getValueOrDefault(
                        e8.modify(nationkey));
...
std::unique_ptr<Application> createApplication() {
  return std::make_unique<TpchFQ2FactorizedJoinApplication>();
}
