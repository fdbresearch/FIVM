...
class TpchFQ4FactorizedJoinApplication : public Application {
 public:
  ...
  void onEndProcessing(const dbtoaster::data_t& data,
    ...
    std::cout << "  sizeof(V_suppkey_LPS1) = "
              << data.get_V_suppkey_LPS1().store.size() << std::endl;
    std::cout << "  sizeof(V_partkey_LP1) = "
              << data.get_V_partkey_LP1().count() << std::endl;
    std::cout << "  sizeof(V_l_orderkey_L1) = "
              << data.get_V_l_orderkey_L1().count() << std::endl;
    std::cout << "  sizeof(V_ps_availqty_P1) = "
              << data.get_V_ps_availqty_P1().count() << std::endl;
    std::cout << "  sizeof(V_s_name_S1) = " << data.get_V_s_name_S1().count()
              << std::endl;
    ...
    for (auto& t1 : v_suppkey_LPS1.store) {
      ...
      {  // For each V_partkey_LP1(suppkey) -> partkey
        ...
        const auto& rel2 =
            data.get_V_partkey_LP1().getValueOrDefault(e2.modify(suppkey));
        for (auto& t2 : rel2.store) {
          auto& key2 = t2.first;
          auto partkey = std::get<0>(key2);

          // For each V_l_orderkey_L1(partkey, suppkey) -> (l_orderkey,
          // l_linenumber, l_quantity, l_extendedprice, l_discount, l_tax,
          // l_returnflag, l_linestatus, l_shipdate, l_commitdate,
          // l_receiptdate, l_shipinstruct, l_shipmode, l_comment)
          V_l_orderkey_L1_entry e3;
          const auto& rel3 = data.get_V_l_orderkey_L1().getValueOrDefault(
              e3.modify(partkey, suppkey));
          for (auto& t3 : rel3.store) {
            ...
            const auto& rel4 = data.get_V_ps_availqty_P1().getValueOrDefault(
                e4.modify(partkey, suppkey));
            for (auto& t4 : rel4.store) {
              ...
              const auto& rel5 =
                  data.get_V_s_name_S1().getValueOrDefault(e5.modify(suppkey));
...
std::unique_ptr<Application> createApplication() {
  return std::make_unique<TpchFQ4FactorizedJoinApplication>();
}
