#ifndef APPLICATION_TPCH_FQ2_LISTING_JOIN_HPP
#define APPLICATION_TPCH_FQ2_LISTING_JOIN_HPP
...
class TpchFQ2ListingJoinApplication : public Application {
 public:
  ...
  void onEndProcessing(const dbtoaster::data_t& data,
    ...
    size_t output_size = v_orderkey_LPCNO1.store.size();
    size_t total_multiplicity = 0;

    for (auto& t : v_orderkey_LPCNO1.store) {
...
std::unique_ptr<Application> createApplication() {
  return std::make_unique<TpchFQ2ListingJoinApplication>();
}

#endif /* APPLICATION_TPCH_FQ2_LISTING_JOIN_HPP */