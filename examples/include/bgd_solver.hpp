...
struct BGDSolver {
  ...
  */
  ...
  //     const double *sum2_scalar_array_iter = cofactor.scalar_array.data() +
  //     SZ; for (size_t i = 0; i < SZ; i++) {
  ...
  void run(size_t& num_iterations, double& error) {
    ...
    do {
      ...
      /* Backtracking Line Search: Decrease step_size until condition is
       * satisfied */
      size_t backtracking_steps = 0;
      while (error > prev_error - (step_size / 2) * gradient_norm &&
             backtracking_steps < 500) {
