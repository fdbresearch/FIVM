#ifndef RINGRELATIONAL2_HPP
#define RINGRELATIONAL2_HPP
...
struct VectorRelation {
    ...
    VectorRelation& operator+=(const VectorRelation<Idx, Keys...>& other) {
        for (auto &t : other.store) {
            bool found = false;

            for (size_t j = 0; !found && j < store.size(); j++) {
                if (std::get<0>(store[j]) == std::get<0>(t)) {
                    std::get<1>(store[j]) += std::get<1>(t);
...
template <size_t Idx, typename... Keys>
VectorRelation<Idx, Keys...> operator*(long alpha, const VectorRelation<Idx, Keys...>& r) {
...
template <size_t Idx, typename... Keys>
using RingRelation = VectorRelation<Idx, Keys...>;

#endif /* RINGRELATIONAL2_HPP */