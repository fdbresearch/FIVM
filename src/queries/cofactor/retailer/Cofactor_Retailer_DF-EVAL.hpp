...
namespace dbtoaster {
  ...
  typedef MultiHashMap<AGG_INVENTORY_entry, AGG_Inventory_value,
    PrimaryHashIndex<AGG_INVENTORY_entry, AGG_Inventory_mapkey0123_idxfn>
  > AGG_Inventory_map;
      ...
      {  //
        ...
        { //foreach
          ...
          while(e1){
            ...
            AGG_Inventory.addOrDelOnZero(se1, sv1);
        ...
        AGG_InvItem.clear();
        { //foreach
          AGG_INVENTORY_entry* e20 = AGG_Inventory.head;
          while(e20){
            long locn = e20->locn;
            ...
            AGG_InvItem.addOrDelOnZero(se7, sv7);

            e20 = e20->nxt;            
          }
        }

        AGG_InvItemWeat.clear();
        { //foreach
          AGG_InvITEM_entry* e20 = AGG_InvItem.head;
          while(e20){
            long locn = e20->locn;
            long dateid = e20->dateid;
            ...
            AGG_InvItemWeat.addOrDelOnZero(se8, sv8);
        ...
        // AGG_Output.clear();
        { //foreach
          AGG_InvItemWeat_entry* e20 = AGG_InvItemWeat.head;
          while(e20){
            long locn = e20->locn;
        ...
        //     AGG_Output.count += count1 * count2 * count3 * count4;

        //     size_t indexOutput = 0;
        //     AGG_Output.cofactors[indexOutput++] += count1 * count2 * count3 * count4 * locn * locn;
        //     AGG_Output.cofactors[indexOutput++] += count1 * count2 * count3 * count4 * locn * dateid;
        //     AGG_Output.cofactors[indexOutput++] += count1 * count2 * count3 * count4 * locn * ksn;
        //     for (size_t i = 0; i < AGG_INVENTORY_SIZE; i++) {
        //       AGG_Output.cofactors[indexOutput++] += count2 * count3 * count4 * locn * vInventory.sums[i];
        //     }
        //     for (size_t i = 0; i < AGG_ITEM_SIZE; i++) {
        //       AGG_Output.cofactors[indexOutput++] += count1 * count3 * count4 * locn * vItem.sums[i];
        //     }
        //     for (size_t i = 0; i < AGG_WEATHER_SIZE; i++) {
        //       AGG_Output.cofactors[indexOutput++] += count1 * count2 * count4 * locn * vWeather.sums[i];
        //     }
        //     for (size_t i = 0; i < AGG_LC_SIZE; i++) {
        //       AGG_Output.cofactors[indexOutput++] += count1 * count2 * count3 * locn * vLC.sums[i];
        //     }

        //     AGG_Output.cofactors[indexOutput++] += count1 * count2 * count3 * count4 * dateid * dateid;
        //     AGG_Output.cofactors[indexOutput++] += count1 * count2 * count3 * count4 * dateid * ksn;
        //     for (size_t i = 0; i < AGG_INVENTORY_SIZE; i++) {
        //       AGG_Output.cofactors[indexOutput++] += count2 * count3 * count4 * dateid * vInventory.sums[i];
        //     }
        //     for (size_t i = 0; i < AGG_ITEM_SIZE; i++) {
        //       AGG_Output.cofactors[indexOutput++] += count1 * count3 * count4 * dateid * vItem.sums[i];
        //     }
        //     for (size_t i = 0; i < AGG_WEATHER_SIZE; i++) {
        //       AGG_Output.cofactors[indexOutput++] += count1 * count2 * count4 * dateid * vWeather.sums[i];
        //     }
        //     for (size_t i = 0; i < AGG_LC_SIZE; i++) {
        //       AGG_Output.cofactors[indexOutput++] += count1 * count2 * count3 * dateid * vLC.sums[i];
        //     }

        //     AGG_Output.cofactors[indexOutput++] += count1 * count2 * count3 * count4 * ksn * ksn;
        //     for (size_t i = 0; i < AGG_INVENTORY_SIZE; i++) {
        //       AGG_Output.cofactors[indexOutput++] += count2 * count3 * count4 * ksn * vInventory.sums[i];
        //     }
        //     for (size_t i = 0; i < AGG_ITEM_SIZE; i++) {
        //       AGG_Output.cofactors[indexOutput++] += count1 * count3 * count4 * ksn * vItem.sums[i];
        //     }
        //     for (size_t i = 0; i < AGG_WEATHER_SIZE; i++) {
        //       AGG_Output.cofactors[indexOutput++] += count1 * count2 * count4 * ksn * vWeather.sums[i];
        //     }
        //     for (size_t i = 0; i < AGG_LC_SIZE; i++) {
        //       AGG_Output.cofactors[indexOutput++] += count1 * count2 * count3 * ksn * vLC.sums[i];
        //     }

        //     size_t indexInput = 0;
        //     for (size_t i = 0; i < AGG_INVENTORY_SIZE; i++) {
        //       for (size_t j = i; j < AGG_INVENTORY_SIZE; j++) {
        //         AGG_Output.cofactors[indexOutput++] += count2 * count3 * count4 * vInventory.cofactors[indexInput++];
        //       }
        //       for (size_t j = 0; j < AGG_ITEM_SIZE; j++) {
        //         AGG_Output.cofactors[indexOutput++] += count3 * count4 * vInventory.sums[i] * vItem.sums[j]; 
        //       }
        //       for (size_t j = 0; j < AGG_WEATHER_SIZE; j++) {
        //         AGG_Output.cofactors[indexOutput++] += count2 * count4 * vInventory.sums[i] * vWeather.sums[j]; 
        //       }
        //       for (size_t j = 0; j < AGG_LC_SIZE; j++) {
        //         AGG_Output.cofactors[indexOutput++] += count2 * count3 * vInventory.sums[i] * vLC.sums[j]; 
        //       }
        //     }
        ...
        //     for (size_t i = 0; i < AGG_ITEM_SIZE; i++) {
        //       for (size_t j = i; j < AGG_ITEM_SIZE; j++) {
        //         AGG_Output.cofactors[indexOutput++] += count1 * count3 * count4 * vItem.cofactors[indexInput++];
        //       }
        //       for (size_t j = 0; j < AGG_WEATHER_SIZE; j++) {
        //         AGG_Output.cofactors[indexOutput++] += count1 * count4 * vItem.sums[i] * vWeather.sums[j]; 
        //       }
        //       for (size_t j = 0; j < AGG_LC_SIZE; j++) {
        //         AGG_Output.cofactors[indexOutput++] += count1 * count3 * vItem.sums[i] * vLC.sums[j]; 
        //       }
        //     }
        ...
        //     for (size_t i = 0; i < AGG_WEATHER_SIZE; i++) {
        //       for (size_t j = i; j < AGG_WEATHER_SIZE; j++) {
        //         AGG_Output.cofactors[indexOutput++] += count1 * count2 * count4 * vWeather.cofactors[indexInput++];
        //       }
        //       for (size_t j = 0; j < AGG_LC_SIZE; j++) {
        //         AGG_Output.cofactors[indexOutput++] += count1 * count2 * vWeather.sums[i] * vLC.sums[j]; 
        //       }
        //     }

        //     indexInput = 0;
        //     for (size_t i = 0; i < AGG_LC_SIZE; i++) {
        //       for (size_t j = i; j < AGG_LC_SIZE; j++) {
        //         AGG_Output.cofactors[indexOutput++] += count1 * count2 * count3 * vLC.cofactors[indexInput++];
        //       }
        //     }
  ...
  private:  
    ...
    AGG_LC_entry se6;
    AGG_InvITEM_entry se7;
    AGG_InvItemWeat_entry se8;

    AGG_Inventory_value sv1;
    AGG_Location_value sv2;
    AGG_Census_value sv3;
    AGG_Item_value sv4;
    AGG_Weather_value sv5;
    AGG_LC_value sv6;
    AGG_InvItem_value sv7;
    AGG_InvItemWeat_value sv8;
