...
DECLARE MAP AGG_B(float)[][a:float, Deg_b:int, Deg_c:int, Deg_d:int] := 0;
...
ON SYSTEM READY {
    ...
    AGG_B(float)[][a, Deg_b, Deg_c, Deg_d] := 
        AggSum([a, Deg_b, Deg_c, Deg_d],
            AGG_C(float)[][a, b, Deg_c, Deg_d] * TWITTER1(a, b) * DEGREE1(Deg_b) *
            {(Deg_b + Deg_c + Deg_d) <= 2} *
    ...
    AGG(float)[][Deg_a, Deg_b, Deg_c, Deg_d] := 
        AggSum([Deg_a, Deg_b, Deg_c, Deg_d],
            AGG_B(float)[][A, Deg_b, Deg_c, Deg_d] * DEGREE1(Deg_a) * 
