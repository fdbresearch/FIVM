...
namespace dbtoaster {
  ...
  struct data_t : tlq_t{
    ...
    void on_batch_update_INVENTORY(const std::vector<INVENTORY_entry>::iterator &begin, const std::vector<INVENTORY_entry>::iterator &end) {
      ...
      { //foreach
        KEY_DATEID_INVENTORY_ITEM_WEATHER* e5 = DELTA_AGG_InventoryItemWeather.head;
        while (e5) {
          AGG_InventoryItemWeather.addOrDelOnZero(*e5, e5->__av);
          e5 = e5->nxt;
        }
      }
      { //foreach
        KEY_KSN_INVENTORY_ITEM* e6 = DELTA_AGG_InventoryItem.head;
        while (e6) {
          AGG_InventoryItem.addOrDelOnZero(*e6, e6->__av);
          e6 = e6->nxt;
        }
      }
      { //foreach
        KEY_INVENTORYUNITS_INVENTORY* e7 = DELTA_AGG_Inventory.head;
        while (e7) {
          AGG_Inventory.addOrDelOnZero(*e7, e7->__av);
          e7 = e7->nxt;
        }
      }
    }
    
    void on_batch_update_LOCATION(const std::vector<LOCATION_entry>::iterator &begin, const std::vector<LOCATION_entry>::iterator &end) {
      tN += std::distance(begin, end);
      DELTA_AGG_Location.clear();
      { //foreach
        for (std::vector<DELTA_LOCATION_entry>::iterator e8 = begin; e8 != end; e8++) {
          // Create key
          ...
          DELTA_AGG_Location.addOrDelOnZero(key_rgncd_location, payload_rgncd_location_map);
        }
      }
      DELTA_AGG_LocationCensus.clear();
      { //foreach
        KEY_RGNCD_LOCATION* e9 = DELTA_AGG_Location.head;
        while (e9) {
          long locn = e9->locn;
          long zip = e9->zip;
          ...
          if (pCensus.count() > 0) {
            ...
            while (p1) {
              ...
              while (p2) {
                ...
                DELTA_AGG_LocationCensus.addOrDelOnZero(key_zip_location_census, payload_zip_location_census_map);
      ...
      { //foreach
        KEY_ZIP_LOCATION_CENSUS* e10 = DELTA_AGG_LocationCensus.head;
        while (e10) {          
          long locn = e10->locn;
          const PAYLOAD_ZIP_LOCATION_CENSUS_map &pLocationCensus = e10->__av;
          const PAYLOAD_DATEID_INVENTORY_ITEM_WEATHER_map &pInventoryItemWeather = AGG_InventoryItemWeather.getValueOrDefault(key_dateid_inventory_item_weather.modify(locn));

          if (pInventoryItemWeather.count() > 0) {

            payload_locn.locn = locn;

            PAYLOAD_ZIP_LOCATION_CENSUS* p1 = pLocationCensus.head;
            while (p1) {
              payload_locn.zip = p1->zip;
              payload_locn.rgn_cd = p1->rgn_cd;
              payload_locn.clim_zn_nbr = p1->clim_zn_nbr;
              payload_locn.tot_area_sq_ft = p1->tot_area_sq_ft;
              payload_locn.sell_area_sq_ft = p1->sell_area_sq_ft;
              payload_locn.avghhi = p1->avghhi;
              payload_locn.supertargetdistance = p1->supertargetdistance;
              payload_locn.supertargetdrivetime = p1->supertargetdrivetime;
              payload_locn.targetdistance = p1->targetdistance;
              payload_locn.targetdrivetime = p1->targetdrivetime;
              payload_locn.walmartdistance = p1->walmartdistance;
              payload_locn.walmartdrivetime = p1->walmartdrivetime;
              payload_locn.walmartsupercenterdistance = p1->walmartsupercenterdistance;
              payload_locn.walmartsupercenterdrivetime = p1->walmartsupercenterdrivetime;
              payload_locn.population = p1->population;
              payload_locn.white = p1->white;
              payload_locn.asian = p1->asian;
              payload_locn.pacific = p1->pacific;
              payload_locn.blackafrican = p1->blackafrican;
              payload_locn.medianage = p1->medianage;
              payload_locn.occupiedhouseunits = p1->occupiedhouseunits;
              payload_locn.houseunits = p1->houseunits;
              payload_locn.families = p1->families;
              payload_locn.households = p1->households;
              payload_locn.husbwife = p1->husbwife;
              payload_locn.males = p1->males;
              payload_locn.females = p1->females;
              payload_locn.householdschildren = p1->householdschildren;
              payload_locn.hispanic = p1->hispanic;
              payload_locn.__av = p1->__av;


              PAYLOAD_DATEID_INVENTORY_ITEM_WEATHER* p2 = pInventoryItemWeather.head;
              while (p2) {
                payload_locn.dateid = p2->dateid;
                payload_locn.ksn = p2->ksn;
                payload_locn.inventoryunits = p2->inventoryunits;
                payload_locn.subcategory = p2->subcategory;
                payload_locn.category = p2->category;
                payload_locn.categorycluster = p2->categorycluster;
                payload_locn.prize = p2->prize;
                payload_locn.rain = p2->rain;
                payload_locn.snow = p2->snow;
                payload_locn.maxtemp = p2->maxtemp;
                payload_locn.mintemp = p2->mintemp;
                payload_locn.meanwind = p2->meanwind;
                payload_locn.thunder = p2->thunder;
                payload_locn.__av *= p2->__av;
      ...
      { //foreach
        KEY_ZIP_LOCATION_CENSUS* e7 = DELTA_AGG_LocationCensus.head;
        while (e7) {
          AGG_LocationCensus.addOrDelOnZero(*e7, e7->__av);
          e7 = e7->nxt;
        }
      }      
      { //foreach
        KEY_RGNCD_LOCATION* e7 = DELTA_AGG_Location.head;
        while (e7) {
          AGG_Location.addOrDelOnZero(*e7, e7->__av);
          e7 = e7->nxt;
        }
      }
    }
    ...
    void on_batch_update_CENSUS(const std::vector<CENSUS_entry>::iterator &begin, const std::vector<CENSUS_entry>::iterator &end) {
      ...
      { //foreach
        for (std::vector<DELTA_CENSUS_entry>::iterator e13 = begin; e13 != end; e13++) {
          // Create key
          ...
          DELTA_AGG_Census.addOrDelOnZero(key_population_census, payload_population_census_map);
        }
      }

      DELTA_AGG_LocationCensus.clear();
      { //foreach
        KEY_POPULATION_CENSUS* e14 = DELTA_AGG_Census.head;
        while (e14) {
          long zip = e14->zip;
          const PAYLOAD_POPULATION_CENSUS_map &pCensus = e14->__av;

          { //slice
            const SecondaryIdxNode<KEY_RGNCD_LOCATION>* n15_head = static_cast<const SecondaryIdxNode<KEY_RGNCD_LOCATION>*>(AGG_Location.slice(key_rgncd_location.modify1(zip), 0));
            const SecondaryIdxNode<KEY_RGNCD_LOCATION>* n15 = n15_head;
            KEY_RGNCD_LOCATION* e15;
            while (n15) {
              e15 = n15->obj;
              long locn = e15->locn;
              const PAYLOAD_RGNCD_LOCATION_map &pLocation = e15->__av;
      ...
      { //foreach
        KEY_ZIP_LOCATION_CENSUS* e16 = DELTA_AGG_LocationCensus.head;
        while (e16) {
          long locn = e16->locn;          
          const PAYLOAD_ZIP_LOCATION_CENSUS_map &pLocationCensus = e16->__av;
          const PAYLOAD_DATEID_INVENTORY_ITEM_WEATHER_map &pInventoryItemWeather = AGG_InventoryItemWeather.getValueOrDefault(key_dateid_inventory_item_weather.modify(locn));
      ...
      { //foreach
        KEY_POPULATION_CENSUS* e7 = DELTA_AGG_Census.head;
        while (e7) {
          AGG_Census.addOrDelOnZero(*e7, e7->__av);
          e7 = e7->nxt;
        }
      }
    }
    ...
    void on_batch_update_ITEM(const std::vector<ITEM_entry>::iterator &begin, const std::vector<ITEM_entry>::iterator &end) {
      ...
      { //foreach
        for (std::vector<DELTA_ITEM_entry>::iterator e19 = begin; e19 != end; e19++) {
          // Create key
          ...
          DELTA_AGG_Item.addOrDelOnZero(key_subcategory_item, payload_subcategory_item_map);
        }
      }

      DELTA_AGG_InventoryItem.clear();
      { //foreach
        KEY_SUBCATEGORY_ITEM* e20 = DELTA_AGG_Item.head;
        while (e20) {
          DOUBLE_TYPE ksn = e20->ksn;
          const PAYLOAD_SUBCATEGORY_ITEM_map & pItem = e20->__av;

          { //slice
            const SecondaryIdxNode<KEY_INVENTORYUNITS_INVENTORY>* n21_head = static_cast<const SecondaryIdxNode<KEY_INVENTORYUNITS_INVENTORY>*>(AGG_Inventory.slice(key_inventoryunits_inventory.modify2(ksn), 0));
            const SecondaryIdxNode<KEY_INVENTORYUNITS_INVENTORY>* n21 = n21_head;
            KEY_INVENTORYUNITS_INVENTORY* e21;
            while (n21) {
              e21 = n21->obj;
              long locn = e21->locn;
              long dateid = e21->dateid;
              const PAYLOAD_INVENTORYUNITS_INVENTORY_map &pInventory = e21->__av;
              ...
              n21 = (n21 != n21_head ? n21->nxt : n21->child);
            }
          }
          e20 = e20->nxt;
        }
      ...
      { //foreach
        KEY_SUBCATEGORY_ITEM* e7 = DELTA_AGG_Item.head;
        while (e7) {
          AGG_Item.addOrDelOnZero(*e7, e7->__av);
          e7 = e7->nxt;
        }
      }
    }
    ...
    void on_batch_update_WEATHER(const std::vector<WEATHER_entry>::iterator &begin, const std::vector<WEATHER_entry>::iterator &end) {
      ...
      { //foreach
        for (std::vector<DELTA_WEATHER_entry>::iterator e27 = begin; e27 != end; e27++) {
          // Create key
          ...
          DELTA_AGG_Weather.addOrDelOnZero(key_rain_weather, payload_rain_weather_map);
        }
      }

      DELTA_AGG_InventoryItemWeather.clear();
      { //foreach
        KEY_RAIN_WEATHER* e3 = DELTA_AGG_Weather.head;
        while (e3) {
          long locn = e3->locn;
          long dateid = e3->dateid;
          const PAYLOAD_RAIN_WEATHER_map &pWeather = e3->__av;
          const PAYLOAD_KSN_INVENTORY_ITEM_map &pInventoryItem = AGG_InventoryItem.getValueOrDefault(key_ksn_inventory_item.modify(locn, dateid));

          if (pInventoryItem.count() > 0) {
      ...
      { //foreach
        KEY_RAIN_WEATHER* e7 = DELTA_AGG_Weather.head;
        while (e7) {
          AGG_Weather.addOrDelOnZero(*e7, e7->__av);
          e7 = e7->nxt;
        }
      }
    }
  ...
  private:
    ...
    AGG_INVENTORYUNITS_INVENTORY_map AGG_Inventory;
    AGG_INVENTORYUNITS_INVENTORY_map DELTA_AGG_Inventory;

    AGG_SUBCATEGORY_ITEM_map AGG_Item;
    AGG_SUBCATEGORY_ITEM_map DELTA_AGG_Item;

    AGG_RAIN_WEATHER_map AGG_Weather;
    AGG_RAIN_WEATHER_map DELTA_AGG_Weather;

    AGG_RGNCD_LOCATION_map AGG_Location;
    AGG_RGNCD_LOCATION_map DELTA_AGG_Location;

    AGG_POPULATION_CENSUS_map AGG_Census;
    AGG_POPULATION_CENSUS_map DELTA_AGG_Census;

    AGG_KSN_INVENTORY_ITEM_map AGG_InventoryItem;
    AGG_KSN_INVENTORY_ITEM_map DELTA_AGG_InventoryItem;

    AGG_DATEID_INVENTORY_ITEM_WEATHER_map AGG_InventoryItemWeather;
    AGG_DATEID_INVENTORY_ITEM_WEATHER_map DELTA_AGG_InventoryItemWeather;

    AGG_ZIP_LOCATION_CENSUS_map AGG_LocationCensus;
    AGG_ZIP_LOCATION_CENSUS_map DELTA_AGG_LocationCensus;
