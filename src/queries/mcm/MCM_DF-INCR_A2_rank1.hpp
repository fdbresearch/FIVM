...
namespace dbtoaster {
  ...
  struct data_t : tlq_t{
    ...
    void on_batch_update_A2(const std::vector<A2_entry>::iterator &begin, const std::vector<A2_entry>::iterator &end) {
      ...
      for (int j = 0; j < matrixSize; j++) {
        se3.i = j;
        se3.__av = (DOUBLE_TYPE) rand() / RAND_MAX;
        DELTA_A2_V.insert(se3);
      }

      tN += DELTA_A2_U.count() * DELTA_A2_V.count();

      // cout << "u = [ ";
      // for (int i = 0; i < matrixSize; i++) {
      //   cout << DELTA_A2_U.getValueOrDefault(se3.modify(i)) << " ";
      // }
      // cout << "];" << endl;
      // cout << "v = [ ";
      // for (int i = 0; i < matrixSize; i++) {
      //   cout << DELTA_A2_V.getValueOrDefault(se3.modify(i)) << " ";
      // }
      // cout << "];" << endl;
      ...
      { //foreach
        ...
        while (e3) {
          ...
          agg1.add(st2.modify(i), (v3 * DELTA_A2_U.getValueOrDefault(se3.modify(j))));
          e3 = e3->nxt;
        }
      }
      ...
      { //foreach
        ...
        while (e3) {
          ...
          long l = e3->j;
          DOUBLE_TYPE v3 = e3->__av;
          agg2.add(st2.modify(l), (v3 * DELTA_A2_V.getValueOrDefault(se3.modify(k))));
          e3 = e3->nxt;
        }
      }
      ...
      { //temp foreach
        DELTA_A2_entry* u = DELTA_A2_U.head;
        while(u) {
          long i = u->i;
          DOUBLE_TYPE v1 = u->__av;

          { //temp foreach
            DELTA_A2_entry* v = DELTA_A2_V.head;
            while(v) {
              long j = v->i;
              DOUBLE_TYPE v2 = v->__av;
              BASE_A2.addOrDelOnZero(se4.modify(i, j), v1 * v2);
              v = v->nxt;
            }
          }       
          u = u->nxt;
  ...
  private:
    ...
    A3_map A3;
    BASE_A2_map BASE_A2;
    DELTA_A2_map DELTA_A2_U;
    DELTA_A2_map DELTA_A2_V;
