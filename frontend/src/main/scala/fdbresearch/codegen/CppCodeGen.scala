...
class CppCodeGen(val cgOpts: CodeGenOptions) extends ICppCodeGen

trait ICppCodeGen extends CodeGen {

  import scala.collection.mutable.{ HashMap, HashSet, ArrayBuffer }
  import fdbresearch.util.Utils._
  import fdbresearch.util.TypeHelper
  import fdbresearch.util.TypeHelper.Cpp._
  import fdbresearch.core._
  import fdbresearch.core.M3._ 
  import CppCodeGen._
  ...
  protected def applyFnName(n: String, targs: Option[List[String]]) =
    n + targs.map(x => "<" + x.mkString(", ") + ">").mkString
  ...
  private def emitConstInits = 
    hoistedConsts.map { case (Apply(fn, _, args, targs), n) => 
      val fnName = applyFnName(fn, targs)
      if (fn == "STRING_TYPE") {      // string initialization
        assert(args.size == 1)        // sanity check
        n + " = " + fnName + "(\"" + args.head.asInstanceOf[Const].v + "\");"
      }
      else {
        val sArgs = args.map {
  ...
  private def emitIVMStructure(s0: System) = {
    ...
    val sRegisterData = stringIf(!cgOpts.useExperimentalRuntimeLibrary, {
  ...
  private def cpsExpr(ex: Expr, co: String => String = (v: String) => v, am: Option[List[(String, Type)]] = None): String = ex match {
    case Ref(n, _) => co(rn(n))

    case Const(tp, v) => tp match {      
      case TypeLong => co(TypeHelper.fromString(v, tp) + "L")
      case TypeFloat => co(TypeHelper.fromString(v, tp) + "f")
      case TypeChar => co("'" + TypeHelper.fromString(v, tp) + "'")
    ...
    case Apply(fn, tp, as, tas) => applyFunc(co, applyFnName(fn, tas), tp, as)
